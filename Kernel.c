code Kernel
  ...
  -- Harris Adam Charron, 1000413572
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
	...
	oldIntStat, numberFramesNeeded, i: int
	newAddrSpace: AddrSpace = new AddrSpace
	oldFrameAddr, newFrameAddr, oldUserPC: int
      
--      print("\nHandle_Sys_Fork invoked!\n\n")
      
      newThread = threadManager.GetANewThread()
      --newThread.Init("childThread")
      newThread.name = currentThread.name
      newThread.status = currentThread.status
      ...
      newAddrSpace.Init()
      newThread.myProcess.addrSpace = newAddrSpace
      numberFramesNeeded = currentThread.myProcess.addrSpace.numberOfPages
      frameManager.GetNewFrames(&newThread.myProcess.addrSpace, numberFramesNeeded)
      
      --Store fileDescriptor in child as well
	...
	if currentThread.myProcess.fileDescriptor[i] != null
	  newThread.myProcess.fileDescriptor[i].numberOfUsers = newThread.myProcess.fileDescriptor[i].numberOfUsers + 1 
	endIf
      endFor
      fileManager.fileManagerLock.Unlock()
      
      for i = 0 to numberFramesNeeded - 1
	oldFrameAddr = (currentThread.myProcess.addrSpace.ExtractFrameAddr(i))
	newFrameAddr = (newThread.myProcess.addrSpace.ExtractFrameAddr(i))
	MemoryCopy(newFrameAddr, oldFrameAddr, PAGE_SIZE)
	if currentThread.myProcess.addrSpace.IsWritable(i)
	  newThread.myProcess.addrSpace.SetWritable(i)
	else
	  newThread.myProcess.addrSpace.ClearWritable(i)
	endIf
      endFor
  ...
  function ResumeChildAfterFork(oldUserPC: int)
      -- Then newly started thread needs to do the following:
	...
	initSystemStackTop: ptr to int
      ...
      --printInt(currentThread.stackTop)
      --InitUserStackTop = currentThread.stackTop
      --BecomeUserThread(InitUserStackTop, oldUserPC, initSystemStackTop asInteger)
      BecomeUserThread(currentThread.userRegs[14], oldUserPC, initSystemStackTop asInteger)
    endFunction
...
--      print("\n\n")
	...
	if processManager.processTable[i].pid == processID
	  if processManager.processTable[i].status != FREE && processManager.processTable[i].parentsPid == currentThread.myProcess.pid
	    returnVal = processManager.WaitForZombie(&processManager.processTable[i])
	    return returnVal
	  endIf
	endIf
      endFor
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
	...
	strBuffer: array [MAX_STRING_SIZE] of char
      ...
      temp = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
      if (temp < 0)
	return -1
      endIf
	...
	return -1
      ...
      initUserPC = newOpenFile.LoadExecutable(&newAddrSpace)
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
	...
	strBuffer: array [MAX_STRING_SIZE] of char
      ...
      --printHex(filename asInteger)
      temp = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
      if temp == -1	-- See if it was able to get the string from virtual space
	return -1
      endIf
      
      if temp > MAX_STRING_SIZE	-- See if filename is too long
	return -1
      endIf
	...
	if currentThread.myProcess.fileDescriptor[i] == null
	  if StrEqual(&strBuffer, "terminal")
	    ...
	    currentThread.myProcess.fileDescriptor[i].kind = TERMINAL
	    return i
	  endIf
	  newFile = fileManager.Open(&strBuffer)
	  if newFile == null
	    return -1
	  endIf
	  currentThread.myProcess.fileDescriptor[i] = newFile
	  currentThread.myProcess.fileDescriptor[i].kind = FILE
	  return i
	endIf
      endFor
      return -1
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      var 
	readStatus: bool
	virtAddr, destAddr, virtPage, offset, copiedSoFar, thisChunkSize: int
	nextPosInFile, sizeOfFile, initialPosition: int
	i, index: int 
	newChar: char
	--terminalString: array [SERIAL_GET_BUFFER_SIZE] of char
	--temp: int
	oldInt: int
	oldInt = SetInterruptsTo (ENABLED)
      ...
      if (fileDesc < 0 || fileDesc > currentThread.myProcess.fileDescriptor arraySize ||
	currentThread.myProcess.fileDescriptor[fileDesc] == null ||
	(currentThread.myProcess.fileDescriptor[fileDesc] != null && 
	currentThread.myProcess.fileDescriptor[fileDesc].fcb == null &&
	currentThread.myProcess.fileDescriptor[fileDesc].kind != TERMINAL) || 
	sizeInBytes < 0)
	return -1
      endIf
      
      -- If we're reading from the TERMINAL
	...
	while true    
	  ...
	  -- Check for errors...
	  if virtPage < 0 || virtPage > NUMBER_OF_PHYSICAL_PAGE_FRAMES || !currentThread.myProcess.addrSpace.IsWritable(virtPage) || !currentThread.myProcess.addrSpace.IsValid(virtPage)
	  --if virtPage < 0 || virtPage > MAX_PAGES_PER_VIRT_SPACE || !currentThread.myProcess.addrSpace.IsValid(virtPage) || !currentThread.myProcess.addrSpace.IsWritable(virtPage)	    
	    return -1
	  endIf
	  ...
	  for index = 0 to thisChunkSize - 1
	    newChar = serialDriver.GetChar()
	    copiedSoFar = copiedSoFar + 1
	    
	    if newChar == '\r' || newChar == '\n'
	      *(destAddr asPtrTo char + index) = '\n'
	      return copiedSoFar
	    endIf
	    ...
	    *(destAddr asPtrTo char + index) = newChar
	  endFor
	...
	return copiedSoFar
      ...
      initialPosition = currentThread.myProcess.fileDescriptor[fileDesc].currentPos
      for i = 1 to 2
	virtAddr = buffer asInteger
	virtPage = virtAddr / PAGE_SIZE
	offset = virtAddr % PAGE_SIZE
	copiedSoFar = 0
	currentThread.myProcess.fileDescriptor[fileDesc].currentPos = initialPosition
	...
	while true
	  ...
	  if i == 2
	    ...
	    if destAddr == 0
	      return copiedSoFar
	    endIf
	    readStatus = fileManager.SynchRead(currentThread.myProcess.fileDescriptor[fileDesc], destAddr, nextPosInFile, thisChunkSize)
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
	...
	return -1
      ...
      -- If we're writing to the TERMINAL
      if currentThread.myProcess.fileDescriptor[fileDesc].kind == TERMINAL
	--initialPosition = currentThread.myProcess.fileDescriptor[fileDesc].currentPos
	virtAddr = buffer asInteger
	virtPage = virtAddr / PAGE_SIZE
	...
	while true  
	  ...
	  -- Check for errors...
	  if virtPage < 0 || virtPage > NUMBER_OF_PHYSICAL_PAGE_FRAMES || !currentThread.myProcess.addrSpace.IsValid(virtPage)
	    return -1
	  endIf
	  ...
	  for index = 0 to thisChunkSize - 1
	    nextChar = *(destAddr asPtrTo char + index)
	...
	while true
	  ...
	  if i == 2
	    ...
	    if destAddr == 0
	      return copiedSoFar
	    endIf
	    writeStatus = fileManager.SynchWrite(currentThread.myProcess.fileDescriptor[fileDesc], destAddr, nextPosInFile, thisChunkSize)
...
--      print("\n\n")
	...
	return -1
      ...
      if open == null || newCurrentPos < -1 || newCurrentPos > open.fcb.sizeOfFileInBytes
	fileManager.fileManagerLock.Unlock()
	return -1
      endIf
      if newCurrentPos == -1
	-- Go to end of file
	open.currentPos = open.fcb.sizeOfFileInBytes
	fileManager.fileManagerLock.Unlock()
	return open.currentPos
      endIf
...
--      print("\n\n")
	...
	if currentThread.myProcess.fileDescriptor[fileDesc].kind == FILE
	  fileManager.Flush(currentThread.myProcess.fileDescriptor[fileDesc])
...
-----------------------------  OpenFile  ---------------------------------
  ...
  function InitFirstProcess()
    -- Obtains new Thread object by invoking GetANewThread in ThreadManager
    -- Use Thread.Fork() to start it running the StartUserProcess function
  ...
  -- Obtains Process Control Block from ProcessManager
  -- Let PCB and Thread objects point to each other
  -- Open user program file using FileManager.Open()
  -- Use OpenFile.LoadExecutable() to load the program into the PCB's address space
  -- Compute initial values of userStackTop and systemStackTop
  -- Turn this thread to a user-level thread, do a one-way jump to the user level-process,
  -- and never returns
  -- For user-level process to get back into the kernel, it needs to use a syscall trap
  ...
  function StartUserProcess()
    ...
    newPCB = processManager.GetANewProcess()
    currentThread.myProcess = newPCB
    newPCB.myThread = currentThread
    
    newOpenFile = fileManager.Open("TestProgram5")
    initUserPC = newOpenFile.LoadExecutable(&newPCB.addrSpace)
    fileManager.Close(newOpenFile)
    
    InitUserStackTop = ((newPCB.addrSpace.numberOfPages)*PAGE_SIZE)
    initSystemStackTop = &currentThread.systemStack[SYSTEM_STACK_SIZE - 1]
    
    oldIntStat = SetInterruptsTo(DISABLED)
    newPCB.addrSpace.SetToThisPageTable()
    currentThread.isUserThread = true
    BecomeUserThread(InitUserStackTop, initUserPC, initSystemStackTop asInteger)
  endFunction
...
-----------------------------  SerialDriver  ---------------------------------
  ...
  behavior SerialDriver
	...
	--serial_status_word_address = SERIAL_STATUS_WORD_ADDRESS --0x00FFFF00
	--serial_data_word_address = SERIAL_DATA_WORD_ADDRESS --0x00FFFF04
	--((& serial_status_word_address) asPtrTo int) = SERIAL_STATUS_WORD_ADDRESS
	--((& serial_data_word_address) asPtrTo int) = SERIAL_DATA_WORD_ADDRESS
	...
	serialHandlerThread.Init("SerialHandler")
	...
	serialHasBeenInitialized = true
      ...
      -- Passed a character, which it will add to the putBuffer queue
      -- If putBuffer is full, then the method will block
      -- Else, it will return immediately after buffering the character
      -- Does not wait for the I/O to complete
      -- Need to use the serialLock before using either of the buffers
      putBufferSem.Down()	-- Deals with the empty spots there are in putBuffer
      serialLock.Lock()
      ...
      -- Signal serialNeedsAttention which wakes up serialHandlerThread
	...
	-- Get a character from the getBuffer queue and return it
	-- If getBuffer is empty, this method will block and wait for the user to type
	-- a character before returning.
	-- Need to use the serialLock before using either of the buffers
	var returnChar: char
	serialLock.Lock()
	while getBufferSize == 0
	  -- Wait on getCharacterAvail, which will be signaled with Down() operation
	  -- after a new characer is received from the device and added to the buffer
	  getCharacterAvail.Wait(&serialLock)
	endWhile 
	returnChar = getBuffer[getBufferNextOut]
	--getBuffer[getBufferNextOut] = '\0'
	...
	return returnChar
      ...
      -- Need to use the serialLock before using either of the buffers
      -- If user pressed a new key (new char received), fetches the new character and 
      -- moves it into getBuffer
      -- If the serial transmission channel is free (done transmitting the previous
      -- char), and there are more characters waiting in putBuffer to be printed, the 
      -- outputting of the next character must be started and the thread must wake up
      -- any other threads waiting on the getBuffer (becoming non-empty) and the 
      -- putBuffer (becoming non-full).
      -- Code for SerialHandler is an infinite loop that waits on the semaphore, then
      -- checks things, then repeats (going to sleep until the next time the semaphore
      -- is signaled). 
      
      var tempChar: char
	while true
	  serialNeedsAttention.Down() 		-- Wait on serialNeedsAttention
	  -- First, input stream
	  if (*serial_status_word_address & SERIAL_CHARACTER_AVAILABLE_BIT) == 1
	    serialLock.Lock()
	    tempChar = intToChar(*serial_data_word_address)
	    if getBufferSize == SERIAL_GET_BUFFER_SIZE
	      -- If getBuffer is full, print a message about the char being dropped.
	      print("\nSerial input buffer overrun - character '")
	      printChar(tempChar)
	      print("' was ignored\n")
	    else 
	      getBuffer[getBufferNextIn] = tempChar
	  ...
	  if (*serial_status_word_address & SERIAL_OUTPUT_READY_BIT) == 2
	    serialLock.Lock()
	    if putBufferSize > 0
	      tempChar = putBuffer[putBufferNextOut]
	      *serial_data_word_address = charToInt(tempChar)
	      --putBuffer[putBufferNextOut] = '\0'
	      putBufferNextOut = (putBufferNextOut + 1)%SERIAL_PUT_BUFFER_SIZE
	      putBufferSize = putBufferSize - 1
	      -- Signal putBufferSem to wake up PutChar threads waiting to add to buffer
	      putBufferSem.Up()
	    endIf
	    serialLock.Unlock()
	  endIf
...
-------------------------------- SerialHandlerFunction -------------------------------------
  -- Newly created threads from the SerialDriver.Init() method are forked here
  -- They immediately call serialDriver.SerialHandler() and never return
